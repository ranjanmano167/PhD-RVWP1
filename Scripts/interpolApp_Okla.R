library(ggplot2)
source("C:/Users/ci1mmu/QUICS/2_Project_Works/1_Rainfall_Variability/2_Scripts/spaInterpol.R")
directoryData="C:/Users/ci1mmu/QUICS/2_Project_Works/1_Rainfall_Variability/1_Data/1_Rainfall/"
#reading distance matrix,coordinates and event data from corrosponding csv files
distMat=as.matrix(read.csv(paste0(directoryData,"DistanceMatrix.csv"),
                           header=FALSE))
rainGauPos=as.matrix(read.csv(paste0(directoryData,"loc_Okla.csv"),header=TRUE))
eventData=read.csv(paste0(directoryData,"eventData_Okla.csv"),header=TRUE)
...
rgNo=seq(1,50,2) #raingauge numbers  (keep as it is if true is selected above) 
...
# 
# aggregate(list(temperature = eventTS), 
...
#for (k in selEvents) {
  ...
  for (j in timeScaleList) {
    ...
    eventTS=eventTS(directoryData,fileName,eventStart,eventEnd)
    tsInt=rollapply(eventTS,width=5,FUN=mean,by=5,align="right")
    
    tsInt=as.data.frame(tsInt)
    write.csv(tsInt,"int_5min_okla.csv")
    ...
    krigInt=kriging(rainGauPos,avrIntThresHigh,gauNum)
...
for (k in selEvents) {
  ...
  for (j in timeScaleList) {
    ...
    #Extracting time series for a particular time scale
    eventTS=eventTS(directoryData,fileName,eventStart,eventEnd)
    tsInt=rollapply(eventTS,width=60,FUN=mean,by=60,align="right")
